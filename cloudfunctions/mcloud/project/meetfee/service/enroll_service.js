...
class EnrollService extends BaseProjectService {
	...
	async enrollJoin(userId, {
		...
		if (user.USER_LESSON_TOTAL_CNT <= price) this.AppError('您当前的剩余金额不足，暂无法进行预订');
		...
		// 费用统计
		this.editUserLesson(null, userId, -price, LessonLogModel.TYPE.USER_APPT, enrollId, '《' + enroll.ENROLL_CATE_NAME + '-' + enroll.ENROLL_TITLE + '》')
	...
	async cancelMyEnrollJoin(userId, enrollJoinId) {
		...
		await EnrollJoinModel.edit(where, { ENROLL_JOIN_STATUS: EnrollJoinModel.STATUS.CANCEL });

		await this.statEnrollJoin(enrollJoin.ENROLL_JOIN_ENROLL_ID);

		// 费用统计
		await this.editUserLesson(null, userId, enrollJoin.ENROLL_JOIN_PRICE, LessonLogModel.TYPE.USER_CANCEL, enroll._id, '《' + enroll.ENROLL_CATE_NAME + '-' + enroll.ENROLL_TITLE + '》')

	}



	// 修改用户金额 同时进行统计  changeCnt可正负
	async editUserLesson(admin, userId, changeCnt, type, enrollId = '', desc = '') {
		this.AppError('[体育场馆]该功能暂不开放，如有需要请加作者微信：cclinux0730');
	}
