...
class EnrollController extends BaseProjectController {
	...
	async getUsedByDay() {
		...
		for (let k = 0; k < list.length; k++) {
			...
			forms.address = list[k].ENROLL_JOIN_ENROLL_TITLE;
			...
			let node = {
				...
				title: list[k].ENROLL_JOIN_OBJ.name,
	...
	async enrollJoin() {
		...
		let service = new EnrollService();
		return await service.enrollJoin(this._userId, input);
	}
	...
	async getOneLessonLogList() {
		...
		for (let k = 0; k < list.length; k++) {
			...
			list[k].LESSON_LOG_ADD_TIME = timeUtil.timestamp2Time(list[k].LESSON_LOG_ADD_TIME, 'Y-M-D h:m:s');
		}
