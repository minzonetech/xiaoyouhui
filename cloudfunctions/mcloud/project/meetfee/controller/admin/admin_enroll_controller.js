...
class AdminEnrollController extends BaseProjectAdminController {
	...
	async editUseLesson() {
		...
		await service.editUserLesson(this._admin, input.id, input.lessonChangeCnt, type, '', input.lessonDesc);
	}
	...
	async delEnrollJoin() {
		...
		let service = new AdminEnrollService();
		return await service.delEnrollJoin(this._admin, input.enrollJoinId);
	}
	...
	async getTempList() {
		...
		let service = new AdminEnrollService();
		let result = await service.getTempList(input.enrollId);
