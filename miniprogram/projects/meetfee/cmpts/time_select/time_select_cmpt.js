		...
		init: function () {
			...
			for (let j = 0; j < columns.length; j++) {
				...
				for (let k = startTime; k <= endTime; k++) {
					...
					start = dataHelper.padLeft(k, 2, '0') + ':00';

					// 半钟
					if (this.data.timeMode == '48')
						end = dataHelper.padLeft(k, 2, '0') + ':30';
					else
						end = dataHelper.padLeft(k, 2, '0') + ':59';
					...
					let node = {
						...
						idx: k,
						title: dataHelper.padLeft(k, 1, '0') + ':00',
						start,
						end,
						price,
						used: false,
					...
					// 半钟
					if (this.data.timeMode == '48') {
						start = dataHelper.padLeft(k, 2, '0') + ':30';
						end = dataHelper.padLeft(k + 1, 2, '0') + ':00';
						if (end == '24:00') end = '23:59';
						node = {
							enrollId: columns[j].enrollId,
							idx: k,
