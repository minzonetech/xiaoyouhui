...
Page({
	...
	bindLessonCmpt: async function (e) {
		...
		if (!lessonChangeCnt) return pageHelper.showModal('金额不能为空或者小于等于0');
		...
		try {
			await cloudHelper.callCloudSumbit('admin/enroll_user_lesson', params).then(res => {
				...
				pageHelper.showSuccToast('金额' + lessonType + '成功');
			});
	...
	_getSearchMenu: async function () {
		...
		let sortItems1 = [
			{ label: '排序', type: '', value: '' },
			{ label: '余额正序', type: 'sort', value: 'USER_LESSON_TOTAL_CNT|asc' },
			{ label: '余额倒序', type: 'sort', value: 'USER_LESSON_TOTAL_CNT|desc' },
			{ label: '消费额倒序', type: 'sort', value: 'USER_LESSON_USED_CNT|desc' },
			{ label: '消费额倒序', type: 'sort', value: 'USER_LESSON_USED_CNT|desc' },
